...
public class MemoryAccess implements Element{
	...
	public void performMA()
	{
		...
		if (EX_MA_Latch.isMA_enable() && stall)
		{
			...
			if (opcode == 22)
			{
				Simulator.getEventQueue().addEvent(new MemoryReadEvent(Clock.getCurrentTime() + Configuration.mainMemoryLatency,this,containingProcessor.getMainMemory(),result));
			...
			{
				Simulator.getEventQueue().addEvent(new MemoryWriteEvent(Clock.getCurrentTime() + Configuration.mainMemoryLatency ,this,containingProcessor.getMainMemory(),result,rd));
				EX_MA_Latch.setMA_busy(true);
				return;
			}
